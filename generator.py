...
def load_all_anns(gt_paths, dataset='icdar'):
    res = []
    for gt in gt_paths:
        lines = []
...
def load_anns(gt_path, dataset='icdar'):
    lines = []
    ...
    for line in reader:
        ...
        if 'icdar' == dataset:
            try:
                ...
                print('wrong wrong wrong!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1', gt_path)
        else:
            num_points = math.floor((len(line) - 1) / 2) * 2
            poly = np.array(list(map(float, line[:num_points]))).reshape((-1, 2)).tolist()
        if len(poly) < 3:
...
def compute_distance(xs, ys, point_1, point_2):
    ...
    tmp = square_distance_1 * square_distance_2 * square_sin / square_distance
    tmp[tmp<0] = 0
    #if tmp.min() < 0:
    #    print(square_sin.min(), np.abs(cosin).max())
    result = np.sqrt(tmp)
...
def generate(data_dir, batch_size=16, image_size=640, min_text_size=8, shrink_ratio=0.4, thresh_min=0.3,
    ...
    with open(osp.join(data_dir, f'{split}_list.txt')) as f:
        ...
        gt_paths = [osp.join(data_dir, f'{split}_gts', '/'.join([osp.split(image_fname.strip())[0], 'gt_' + osp.split(image_fname.strip())[-1]]).replace('.jpg', '.txt').lstrip('/')) for image_fname in image_fnames]
